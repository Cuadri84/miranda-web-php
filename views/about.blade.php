@extends('layout')
@section('content')
    <section class="main">
      <div class="about__container">
        <h3>THE ULTIMATE LUXURY</h3>
        <h2>About Us</h2>

        <div class="page-navigation">
          <a class="page-navitation__home" href="/index"> Home </a>
          <p>|</p>
          <a class="page-navitation__contact" href="./about"> About </a>
        </div>
      </div>
    </section>

    <section class="welcome">
      <div class="intro-video__item">
        <iframe
          width="100%"
          height="270px"
          src="https://www.youtube.com/embed/Bu3Doe45lcU?start=25&end=75"
          title="Hotel Miranda Video"
          frameborder="0"
          allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
          allowfullscreen
        ></iframe>
      </div>

      <h3>
        Hello. Our hotel has been present for over 20 years. We make the best
        for all our customers.
      </h3>

      <div class="welcome__icons">
        <div class="welcome__icons__item">
          <img src="/assets/about/breakfast.png" />
          <p>Breakfast</p>
        </div>
        <div class="welcome__icons__item">
          <img class="plane-icon" src="/assets/about/airport.png" />
          <p>Airport Pickup</p>
        </div>
        <div class="welcome__icons__item">
          <img src="/assets/about/maps2.png" />
          <p>City Guide</p>
        </div>
        <div class="welcome__icons__item welcome__icons__item--hidden">
          <img src="/assets/about/bbq.png" />
          <p>Bbq Party</p>
        </div>
        <div class="welcome__icons__item">
          <img src="/assets/about/rooml.png" />
          <p>Luxury Room</p>
        </div>
      </div>

      <div class="restaurant__information">
        <div class="restaurant__img">
          <img
            src="/assets/about/restaurant.jpg"
            alt="Hotel Miranda Restaurant"
          />
        </div>
        <div class="restaurant__description">
          <h3>Restaurant</h3>
          <h2>Get Restaurant Facilities & Many Other More</h2>
          <p>
            Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do
            eiusmod tem por incididunt ut labore et dolore magna aliqua. Ut enim
            ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut
            aliquip.
          </p>
          <button>Take a tour</button>
        </div>
      </div>
    </section>

    <section class="core" style="background-color: #222222">
      <h3 style="color: white">Facilities</h3>
      <h2 style="color: white">Core Features</h2>
      <div class="swiper core__swiper">
        <div class="swiper-wrapper">
          <div
            class="swiper-slide core__card"
            style="background-color: #292929"
          >
            <div class="card__icons">
              <img
                class="core__card__icon"
                src="/assets/index/core/hand.png"
                alt="Facilities"
              />
              <img
                class="core__card__icon--number"
                src="/assets/index/core/01black.png"
                alt="Facility 1"
              />
            </div>
            <div class="card__texts">
              <h4 style="color: white">Have High Rating</h4>
              <p style="color: white">
                Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do
                eiusmod tempor incididunt ut labore et dolore magna..
              </p>
            </div>
          </div>
          <div
            class="swiper-slide core__card"
            style="background-color: #292929"
          >
            <div class="card__icons">
              <img
                class="core__card__icon"
                src="/assets/index/core/clock.png"
                alt="Facilities"
              />
              <img
                class="core__card__icon--number"
                src="/assets/index/core/02black.png"
                alt="Facility 1"
              />
            </div>
            <div class="card__texts">
              <h4 style="color: white">Quiet Hours</h4>
              <p style="color: white">
                Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do
                eiusmod tempor incididunt ut labore et dolore magna..
              </p>
            </div>
          </div>
          <div
            class="swiper-slide core__card"
            style="background-color: #292929"
          >
            <div class="card__icons">
              <img
                class="core__card__icon"
                src="/assets/index/core/maps-core.png"
                alt="Facilities"
              />
              <img
                class="core__card__icon--number"
                src="/assets/index/core/03black.png"
                alt="Facility 1"
              />
            </div>
            <div class="card__texts">
              <h4 style="color: white">Best Locations</h4>
              <p style="color: white">
                Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do
                eiusmod tempor incididunt ut labore et dolore magna..
              </p>
            </div>
          </div>
          <div
            class="swiper-slide core__card"
            style="background-color: #292929"
          >
            <div class="card__icons">
              <img
                class="core__card__icon"
                src="/assets/index/core/clock-cancel.png"
                alt="Facilities"
              />
              <img
                class="core__card__icon--number"
                src="/assets/index/core/04black.png"
                alt="Facility 1"
              />
            </div>
            <div class="card__texts">
              <h4 style="color: white">Free Cancellation</h4>
              <p style="color: white">
                Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do
                eiusmod tempor incididunt ut labore et dolore magna..
              </p>
            </div>
          </div>
          <div
            class="swiper-slide core__card"
            style="background-color: #292929"
          >
            <div class="card__icons">
              <img
                class="core__card__icon"
                src="/assets/index/core/card.png"
                alt="Facilities"
              />
              <img
                class="core__card__icon--number"
                src="/assets/index/core/05black.png"
                alt="Facility 1"
              />
            </div>
            <div class="card__texts">
              <h4 style="color: white">Payment Options</h4>
              <p style="color: white">
                Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do
                eiusmod tempor incididunt ut labore et dolore magna..
              </p>
            </div>
          </div>
          <div
            class="swiper-slide core__card"
            style="background-color: #292929"
          >
            <div class="card__icons">
              <img
                class="core__card__icon"
                src="/assets/index/core/ribbon.png"
                alt="Facilities"
              />
              <img
                class="core__card__icon--number"
                src="/assets/index/core/06black.png"
                alt="Facility 1"
              />
            </div>
            <div class="card__texts">
              <h4 style="color: white">Special Offers</h4>
              <p style="color: white">
                Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do
                eiusmod tempor incididunt ut labore et dolore magna..
              </p>
            </div>
          </div>
        </div>
        <div class="swiper-pagination"></div>
      </div>
      <div class="core__grid">
        <div class="swiper-slide core__card" style="background-color: #292929">
          <div class="card__icons">
            <img
              class="core__card__icon"
              src="/assets/index/core/hand.png"
              alt="Facilities"
            />
            <img
              class="core__card__icon--number"
              src="/assets/index/core/01black.png"
              alt="Facility 1"
            />
          </div>
          <div class="card__texts">
            <h4 style="color: white">Have High Rating</h4>
            <p style="color: white">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna..
            </p>
          </div>
        </div>
        <div class="swiper-slide core__card" style="background-color: #292929">
          <div class="card__icons">
            <img
              class="core__card__icon"
              src="/assets/index/core/clock.png"
              alt="Facilities"
            />
            <img
              class="core__card__icon--number"
              src="/assets/index/core/02black.png"
              alt="Facility 1"
            />
          </div>
          <div class="card__texts">
            <h4 style="color: white">Quiet Hours</h4>
            <p style="color: white">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna..
            </p>
          </div>
        </div>
        <div class="swiper-slide core__card" style="background-color: #292929">
          <div class="card__icons">
            <img
              class="core__card__icon"
              src="/assets/index/core/maps-core.png"
              alt="Facilities"
            />
            <img
              class="core__card__icon--number"
              src="/assets/index/core/03black.png"
              alt="Facility 1"
            />
          </div>
          <div class="card__texts">
            <h4 style="color: white">Best Locations</h4>
            <p style="color: white">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna..
            </p>
          </div>
        </div>
        <div class="swiper-slide core__card" style="background-color: #292929">
          <div class="card__icons">
            <img
              class="core__card__icon"
              src="/assets/index/core/clock-cancel.png"
              alt="Facilities"
            />
            <img
              class="core__card__icon--number"
              src="/assets/index/core/04black.png"
              alt="Facility 1"
            />
          </div>
          <div class="card__texts">
            <h4 style="color: white">Free Cancellation</h4>
            <p style="color: white">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna..
            </p>
          </div>
        </div>
        <div class="swiper-slide core__card" style="background-color: #292929">
          <div class="card__icons">
            <img
              class="core__card__icon"
              src="/assets/index/core/card.png"
              alt="Facilities"
            />
            <img
              class="core__card__icon--number"
              src="/assets/index/core/05black.png"
              alt="Facility 1"
            />
          </div>
          <div class="card__texts">
            <h4 style="color: white">Payment Options</h4>
            <p style="color: white">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna..
            </p>
          </div>
        </div>
        <div class="swiper-slide core__card" style="background-color: #292929">
          <div class="card__icons">
            <img
              class="core__card__icon"
              src="/assets/index/core/ribbon.png"
              alt="Facilities"
            />
            <img
              class="core__card__icon--number"
              src="/assets/index/core/06black.png"
              alt="Facility 1"
              style="color: aliceblue"
            />
          </div>
          <div class="card__texts">
            <h4 style="color: white">Special Offers</h4>
            <p style="color: white">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna..
            </p>
          </div>
        </div>
      </div>
    </section>

    <section class="counter">
      <h3>Counter</h3>
      <h2>Some Fun Facts</h2>
      <div class="counter__container__facts">
        <div class="counter__container__item">
          <img src="/assets/about/happy.png" alt="Happy Users Icon" />
          <div class="item-description">
            <p class="item-title">8000</p>
            <p class="item-detail">Happy Users</p>
          </div>
          <img src="/assets/about/arrow.png" alt="Arrow Right Icon" />
        </div>
        <div class="counter__container__item">
          <img src="/assets/about/stars.png" alt="Happy Users Icon" />
          <div class="item-description">
            <p class="item-title">10M</p>
            <p class="item-detail">Reviews & Appriciate</p>
          </div>
          <img src="/assets/about/arrow.png" alt="Arrow Right Icon" />
        </div>
        <div class="counter__container__item">
          <img src="/assets/about/world.png" alt="Happy Users Icon" />
          <div class="item-description">
            <p class="item-title">100</p>
            <p class="item-detail">Country Coverage</p>
          </div>
          <img src="/assets/about/arrow.png" alt="Arrow Right Icon" />
        </div>
      </div>
      <div class="swiper">
        <div class="swiper-wrapper">
          <img class="swiper-slide" src="/assets/about/about1.jpg" alt="" />
          <img class="swiper-slide" src="/assets/about/about2.jpg" alt="" />
        </div>
        <div class="swiper-pagination"></div>
      </div>
      <div class="not-swiper">
        <img class="swiper-slide" src="/assets/about/about1.jpg" alt="" />
        <img class="swiper-slide" src="/assets/about/about2.jpg" alt="" />
      </div>
    </section>
    @endsection